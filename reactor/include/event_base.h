...
#include <iostream>
#include <functional>
using pendingFunc = std::function<void(event_loop *, void *)>;
*/
...
{
    timer_event(timer_callback *timo_cb, void *data, uint64_t arg_ts, uint32_t arg_int = 0) : cb(timo_cb), cb_data(data), ts(arg_ts), interval(arg_int)
    {
    }
