				   ...
				   int tx_buffer_depth, int need_recv_cq)
{
	...
	ctx->send_cq = ibv_exp_create_cq(ctx->context, tx_buffer_depth * user_param->num_of_qps, NULL,
									 ctx->channel, 0, &attr);
	if (!ctx->send_cq)
	{
...
int check_packet_pacing_support(struct pingpong_context *ctx)
{
	...
	return MASK_IS_SET(IBV_EXP_DEVICE_ATTR_PACKET_PACING_CAPS, attr.comp_mask) ? SUCCESS : FAILURE;
}
